...
main_v1_1(void){
	...
	char		*nameOfFunction	= "Tspi_TPM_GetEvent01";
	TSS_HCONTEXT	hContext;
	TSS_RESULT	result;
	TSS_HTPM	hTPM;
	UINT32		ulPcrIndex	= 9;
	UINT32		ulEventNumber	= 0;
	TSS_PCR_EVENT	pcrEventA, pcrEventB;
	BYTE		*pcrData = "random stuff in here";
	UINT32		ulNewPcrValueLength;
	BYTE		*newPcrValue;

	memset(&pcrEventA, 0, sizeof(TSS_PCR_EVENT));
	memset(&pcrEventB, 0, sizeof(TSS_PCR_EVENT));
	...
	pcrEventA.ulPcrIndex = 9;

		//Call PcrExtend
	result = Tspi_TPM_PcrExtend(hTPM, ulPcrIndex, 20, pcrData, &pcrEventA,
	...
	if (result != TSS_SUCCESS) {
		...
		//Get Event
	result = Tspi_TPM_GetEvent(hTPM, ulPcrIndex, ulEventNumber,
					&pcrEventB);
	if (result != TSS_SUCCESS) {
