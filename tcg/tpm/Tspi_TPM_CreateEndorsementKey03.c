...
/*
 * NAME
 *	Tspi_TPM_CreateEndorsementKey03.c
 *
 * DESCRIPTION
 *	This test will return TSS_E_BAD_PARAMETER, because &wrongVar
 *		is passed in as the third parameter of
 *		TPM_CreateEndorsementKey, instead of NULL.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
...
main_v1_1( void )
{
	char			*function = "Tspi_TPM_CreateEndorsementKey03";
	TSS_HCONTEXT		hContext;
	TSS_VALIDATION		wrongVar;
	TSS_HTPM		hTPM;
	TSS_HKEY		hKey;
	TSS_RESULT		result;
	UINT32			exitCode;
	...
	result = Tspi_TPM_CreateEndorsementKey( hTPM, hKey, &wrongVar );
	if ( result != TSS_SUCCESS )
	{
