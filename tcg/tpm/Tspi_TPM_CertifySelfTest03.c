...
/*
 * NAME
 *	Tspi_TPM_CertifySelfTest03.c
 *
 * DESCRIPTION
 *	This test will verify that Tspi_TPM_CertifySelfTest
 *		returns TSS_SUCCESS with use of an authorized key.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
 ...
 *		Get Policy
 *		Set Secret
 *		Create Signing Auth Key Object
 *		Create Signing Key
 *		Load Key
 *		Get Random Number
 *		Set Validation Data
 ...
 * 		Ramon Brandão, ramongb@br.ibm.com, 9/07 - Added authorization data support
 *
 * RESTRICTIONS
 *	None.
 */
...
main_v1_1( void )
{
	char		*function = "Tspi_TPM_CertifySelfTest03";
	...
	ValidationData.rgbExternalData = data;
		...
		//Load Key Blob
	result = Tspi_TPM_CertifySelfTest( hTPM, hMSigningKey, &ValidationData );
	if ( result != TSS_SUCCESS ){
		...
		/* Check the signature */
		result = Testsuite_Verify_Signature(hContext, hMSigningKey, &ValidationData);
		if (result != TSS_SUCCESS) {
