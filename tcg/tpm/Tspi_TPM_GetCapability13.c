...
/*
 * NAME
 *	Tspi_TPM_GetCapability13.c
 *
 * DESCRIPTION
 *	This test will verify that Tspi_TPM_GetCapability receives SUCCESS when the maximum
 *			number of 2048-bit RSA keys that the TPM can support is returned.
...
{
	char			*function = "Tspi_TPM_GetCapability13";
	...
	subCap = TSS_TPMCAP_PROP_MAXKEYS;
	...
	else
	{
		...
		if (pulRespDataLength == sizeof(UINT32))
			fprintf(stderr, "\tThe maximum number of 2048-bit RSA keys that the TPM can support is %u\n\n", *(UINT32 *)pRetData );
	}
