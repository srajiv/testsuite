...
/*
 * NAME
 *	Tspi_TPM_RevokeEndorsementKey01.c
 *
 * DESCRIPTION
...
{
	char			*function = "Tspi_TPM_RevokeEndorsementKey01";
	...
	if ( result != TSS_SUCCESS )
	{
		print_error( "Tspi_CreateRevocableEndorsementKey", result );
		print_error_exit( function, err_string(result) );
		Tspi_Context_FreeMemory( hContext, NULL );
		Tspi_Context_Close( hContext );
		exit( result );
	...
	result = Tspi_TPM_RevokeEndorsementKey( hTPM, resetAuthSize, resetAuth );
	if ( result != TSS_SUCCESS )
	{
