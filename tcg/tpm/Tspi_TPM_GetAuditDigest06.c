...
/*
 * NAME
 *	Tspi_TPM_GetAuditDigest06.c
 *
 * DESCRIPTION
 *	This test will verify that Tspi_TPM_GetAuditDigest
 *		returns TSS_E_BAD_PARAMETER error code if digest size is NULL.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
...
{
	char *			function = "Tspi_TPM_GetAuditDigest06";
	...
	if (result != TSS_SUCCESS) {
		...
		// Call GetAuditDigest
	result = Tspi_TPM_GetAuditDigest(hTPM, NULL_HKEY, FALSE, NULL, &auditDigest,
