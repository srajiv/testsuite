...
/*
 * NAME
 *	Tspi_TPM_Quote2_05.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_TPM_Quote2
 *	The purpose of this test case is to test the versionInfoSize
 *  and the versionInfo output parameters when the fAddVersion is
 *  set to TRUE.
 *
 * ALGORITHM
 *	Setup:
...
int main_v1_2(char version){
	...
	char		*nameOfFunction = "Tspi_TPM_Quote2_05";
	...
	result = Tspi_TPM_Quote2(hTPM, hIdentKey, TRUE, 
				hPcrComposite, NULL,&versionInfoSize,&versionInfo);
	if (result != TSS_SUCCESS) {
		 ...
		 * with the fAddVersion = TRUE */
		if (versionInfoSize > 0){
			if (versionInfo == NULL){
				fprintf(stderr, "Error on VersionInfo (NULL).\n");
				Tspi_Context_Close(hContext);
				exit(1);
			}else{
				fprintf(stdout, "VersionInfo data:\n");
				print_hex(versionInfo, versionInfoSize);
			}
		}else{
			fprintf(stderr, "Error on VersionInfoSize (should be not 0).\n");
			Tspi_Context_Close(hContext);
			exit(1);
		}
