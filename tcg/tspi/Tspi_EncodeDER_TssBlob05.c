...
{
	char		*function = "Tspi_EncodeDER_TssBlob05";
	...
	if ( result != TSS_SUCCESS )
	{
		...
		// EncodeDER_TssBlob (pass in an invalid blob type - above highest value)
	ulEncBlobLength = 0;
	rgbEncBlob = NULL;
	result = Tspi_EncodeDER_TssBlob( ulBlobLength, rgbBlob, TSS_BLOB_TYPE_CMK_BYTE_STREAM + 100, &ulEncBlobLength, rgbEncBlob );
