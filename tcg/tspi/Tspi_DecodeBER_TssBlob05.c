...
{
	char		*function = "Tspi_DecodeBER_TssBlob05";
	...
	if ( result != TSS_SUCCESS )
	{
		...
		// DecodeBER_TssBlob (pass in a buffer that is too small)
	ulDecBlobLength = 10;
	rgbDecBlob = calloc(1, ulDecBlobLength);
	result = Tspi_DecodeBER_TssBlob( 0, rgbEncBlob, &ulBlobType, &ulDecBlobLength, rgbDecBlob );
