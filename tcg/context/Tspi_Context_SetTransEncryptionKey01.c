...
/*
 * NAME
 *	Tspi_Context_SetTransEncryptionKey01.c
 *
 * DESCRIPTION
...
{
	char		*nameOfFunction = "Tspi_Context_SetTransEncryptionKey01";
	...
	if (result != TSS_SUCCESS) {
		...
		//Connect Context
	result = Tspi_Context_CreateObject(hContext, TSS_OBJECT_TYPE_RSAKEY, TSS_KEY_SIZE_512,
					   &hKey);
	if (result != TSS_SUCCESS) {
	...
	result = Tspi_Context_SetTransEncryptionKey(hContext, hKey);
	if (result != TSS_SUCCESS) {
