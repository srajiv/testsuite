...
/*
 * NAME
 *	Tspi_Context_GetRegisteredKeysByUUID2_01.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_Context_GetRegisteredKeysByUUID2.
 *	The purpose of this test is to get TSS_SUCCESS to be 
 *		returned. This is done by carefully following 
 *		the defined algorithm below. 
 *
 * ALGORITHM
 *	Setup:
 *		Create context
...
main_v1_2(char version){
	...
	char		*nameOfFunction = "Tspi_Context_GetRegisteredKeysByUUID2_01";
...
#endif
		//Create Object for the to-be-created key
	...
	if (result != TSS_SUCCESS) {
		...
		//Create the hKey with the hSRK wrapping key - 
		// from the key it takes the context
	...
	if (result != TSS_SUCCESS) {
		...
		//Register the hKey in the system persistent storage
	...
	if (result != TSS_SUCCESS &&
		...
		//Register the hKey - try to register in the user persistent storage
	result = Tspi_Context_RegisterKey(hContext,
	...
	if (result != TSS_SUCCESS &&
		...
		//Get Registered Keys By UUID version 2 - retrieve all the key hierarchy
