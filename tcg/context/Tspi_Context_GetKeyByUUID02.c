...
/*
 * NAME
 *	Tspi_Context_GetKeyByUUID02.c
 *
 * DESCRIPTION
 *	This test will return TSS_E_BAD_PARAMETER, because
 *		TSS_KEY_SIZE_1024 is passed in as the second
 *		parameter to Context_GetKeyByUUID, instead of
 *		TSS_PS_TYPE_SYSTEM.
 *
 * ALGORITHM
 *	Setup:
...
main_v1_1( void )
{
	char		*function = "Tspi_Context_GetKeyByUUID02";
	TSS_HKEY	hSRK;
		...
		(result != TCS_E_KEY_ALREADY_REGISTERED) )
	{
		...
		// get signing key
	result = Tspi_Context_GetKeyByUUID( hContext, TSS_KEY_SIZE_1024,
						migratableSignUUID,
						&hMSigningKey );
