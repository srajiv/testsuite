...
/*
 * NAME
 *	Tspi_Context_GetRegisteredKeysByUUID02.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_Context_GetRegisteredKeysByUUID.
 *	The purpose of this test case is to get TSS_E_INVALID_HANLDE
 *		to be returned. This is accomplished by passing in an
 *		invalid first parameter. In this test case -1 is passed in.
 *
 * ALGORITHM
 *	Setup:
 *		Create
...
main_v1_1(void){
	...
	char		*nameOfFunction = "Tspi_Context_GetRegisteredKeysByUUID02";
	...
	result = Tspi_Policy_SetSecret(keyMigPolicy, 
		TSS_SECRET_MODE_SHA1, 20, TSS_WELL_KNOWN_SECRET);
	if (result != TSS_SUCCESS) {
		print_error("Tspi_Policy_SetSecret ", result);
		print_error_exit(nameOfFunction, err_string(result));
		Tspi_Context_CloseObject(hContext, hKey);
	...
	result = Tspi_Context_GetRegisteredKeysByUUID(-1, 
