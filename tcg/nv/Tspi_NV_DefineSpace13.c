...
/*
 * NAME
 *	Tspi_Nv_DefineSpace13.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_NV_DefineSpace 
 *	with wrong hReadPcrComposite and hWritePcrComposite
 ...
 *		(The Index is 0x0001110e)
 *		(The Permission is 0x2000)
 *		(The datasize is 0xa)
 *		Tspi_NV_DefineSpace(hNVstore, -1, -1)
 *
 *	Test:	
...
{
	char         *nameOfFunction    = "Tspi_Nv_DefineSpace13";
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x0001110e);
	if (result != TSS_SUCCESS)
	{
	...
	if (result != TSS_SUCCESS)
	{
		...
		exit(result);
      	...
      	/*Define NV space*/
	result = Tspi_NV_DefineSpace(hNVStore, -1, -1);
