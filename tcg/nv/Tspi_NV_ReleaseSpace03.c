...
/*
 * NAME
 *	Tspi_Nv_ReleaseSpace03.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_NV_ReleaseSpace with Owner password
 *	The Index is not defined
 *	The purpose of this test case is to get TSS_E_NV_AREA_NOT_EXIST to be returned.
...
{
	char	     *nameOfFunction    = "Tspi_Nv_ReleaseSpace03";
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x00040001);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_DATASIZE, 0, 0x0);
	if (result != TSS_SUCCESS)
	{
...
#ifdef NV_LOCKED	
       if (TSS_ERROR_CODE(result)== TSS_E_NV_AREA_NOT_EXIST)
       {              
