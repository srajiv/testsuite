...
/*
 * NAME
 *	Tspi_Nv_DefineSpace10.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_NV_DefineSpace 
 *	with auth conflict(the permission with both authread and authwrite)
 *	The purpose of this test case is to get TPM_E_AUTH_CONFLICT to be returned.
 ...
 *		(The Index is 0x0001110a)
 *		(The Permission is 0x6)
 *		(The datasize is 0xa)
 *		Tspi_NV_DefineSpace      
 *
 *	Test:	
...
{
	char         *nameOfFunction    = "Tspi_Nv_DefineSpace10";
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x0001110a);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_PERMISSIONS, 0, 0x6);
	if (result != TSS_SUCCESS)
	{
