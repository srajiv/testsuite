...
/*
 * NAME
 *	Tspi_Nv_WriteValue05.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_Nv_WriteValue with password
 *	The offset or Datalength is too big
 *	The purpose of this test case is to get TPM_NOSPACE to be returned.
 ...
 *		(The Index is 0x00011145)
 *		(The Permission is 0x42000)
 *		(The datasize is 0xa)
 *		Tspi_NV_DefineSpace      
 *		Tspi_Context_Create
 *		Tspi_Context_Connect
 ...
 *		Tspi_SetAttribUint32(Index, permission, datasize)
 *		(The Index is 0x00011145)
 *		Tspi_NV_WriteValue(hNVstore, 20, data) 
 *
 *	Test:	
...
{
	char	     *nameOfFunction    = "Tspi_Nv_WriteValue05";
	...
	BYTE         *data              = "12345678901234567890";
	BYTE         policyData[20];
	TSS_RESULT   result;
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x00011145);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_NV_WriteValue(hNVStore, /*offset*/0,/*datalength*/20, data);  
...
#ifdef NV_LOCKED	
       if (TSS_ERROR_CODE(result)== TPM_E_NOSPACE)
       {              
