...
/*
 * NAME
 *	Tspi_Nv_DefineSpace08.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_NV_DefineSpace 
 *	with hReadPcrComposite and hWritePcrComposite
 ...
 *		Create hReadPcrComposite with locality 1,2 
 *		and set the PCR_select 24, 17 with some value
 *		Create hWritePcrComposite with locality 3,4 
 *		and set the PCR_select 1,2 with some value
 *		If nv locked, Setsecret to TPM policy with correct owner passwd
 *		Tspi_SetAttribUint32(Index, permission, datasize)
 *		(The Index is 0x00011108)
...
{
	char	     *nameOfFunction    = "Tspi_Nv_DefineSpace08";

	TSS_HCONTEXT hContext           = NULL_HCONTEXT;
	TSS_HNVSTORE hNVStore           = 0;//NULL_HNVSTORE
	TSS_HPCRS    hWritePcrComposite = NULL_HPCRS;
        TSS_HPCRS    hReadPcrComposite  = NULL_HPCRS;
	TSS_HPOLICY  hPolicy            = NULL_HPOLICY;
	TSS_HTPM     hTPM               = NULL_HTPM;
	BYTE         *pcrValue          = "12345678901234567890";
	UINT32       pcrLen             = 20;
	...
	result = Tspi_Context_CreateObject(hContext, TSS_OBJECT_TYPE_PCRS,
					   TSS_PCRS_STRUCT_INFO_SHORT, &hWritePcrComposite);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_Context_CreateObject", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	/* Set PCR composite object. */
	result = Tspi_Context_CreateObject(hContext, TSS_OBJECT_TYPE_PCRS,
					   TSS_PCRS_STRUCT_INFO_SHORT, &hReadPcrComposite);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_Context_CreateObject", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	result = Tspi_PcrComposite_SetPcrLocality(hReadPcrComposite, 0x1);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_PcrComposite_SetPcrValue(hReadPcrComposite, 23, pcrLen, pcrValue);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_PcrComposite_SetPcrValue(hReadPcrComposite, 17, pcrLen, pcrValue);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_PcrComposite_SetPcrValue", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	result = Tspi_PcrComposite_SetPcrLocality(hWritePcrComposite, 0x1f);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_PcrComposite_SetPcrLocality", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	result = Tspi_PcrComposite_SetPcrValue(hWritePcrComposite, 1, pcrLen, pcrValue);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_PcrComposite_SetPcrValue", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	result = Tspi_PcrComposite_SetPcrValue(hWritePcrComposite, 2, pcrLen, pcrValue);
	if (result != TSS_SUCCESS)
	{
		print_error("Tspi_PcrComposite_SetPcrValue", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x00011108);
	if (result != TSS_SUCCESS)
	{
	...
	printf("read handle=%u, write handle=%u\n", hReadPcrComposite, hWritePcrComposite);
	/*Define NV space*/
	result = Tspi_NV_DefineSpace(hNVStore, hReadPcrComposite, hWritePcrComposite);
