...
/*
 * NAME
 *	Tspi_Nv_DefineSpace11.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_NV_DefineSpace with D bit set
 *	The purpose of this test case is to get TCS_E_BAD_INDEX or TSS_SUCCESS to be returned.
 *	If nv Locked, return TPM_BAD_INDEX, else return TSS_SUCCESS
 ...
 *		(The Index is 0x50000004)
 *		(The Permission is 0x2)
 *		(The datasize is 0xa)
 *		Tspi_NV_DefineSpace
 *
 *	Test:
...
{
	char	     *nameOfFunction    = "Tspi_Nv_DefineSpace11";
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_INDEX, 0,0x50000004);
	if (result != TSS_SUCCESS)
	{
	...
	result = Tspi_SetAttribUint32(hNVStore, TSS_TSPATTRIB_NV_PERMISSIONS, 0, 0x2);
	if (result != TSS_SUCCESS)
	{
