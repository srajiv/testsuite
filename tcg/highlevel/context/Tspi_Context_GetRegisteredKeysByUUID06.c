...
/*
 * NAME
 *	Tspi_Context_GetRegisteredKeysByUUID06.c
 *
 * DESCRIPTION
 *	This test will verify Tspi_Context_GetRegisteredKeysByUUID returns the
 ...
 *	number of children.  Then, call Tspi_Context_GetRegisteredKeysByUUID
...
char *nameOfFunction = "Tspi_Context_GetRegisteredKeysByUUID06";
...
int
verify_hierarchy(struct node *n, TSS_KM_KEYINFO *ppKeyHierarchy, UINT32 size)
{
...
main_v1_1(void)
{
	...
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_SYSTEM, &SRK_UUID,
	...
	/*********  TEST 3 ****************/
	/* Call Tspi_Context_GetRegisteredKeysByUUID on a leaf key of the SRK */
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_SYSTEM,
	...
	/*********  TEST 4 ****************/
	/* Call Tspi_Context_GetRegisteredKeysByUUID on a leaf key of a child of the SRK */
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_SYSTEM,
	...
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_USER, &storage[2].uuid,
						      &pulKeyHierarchySize, &ppKeyHierarchy);
	if (TSS_ERROR_CODE(result) != TSS_E_PS_KEY_NOTFOUND) {
		print_error("Tspi_Context_GetRegisteredKeysByUUID ", result);
		print_error_exit(nameOfFunction, err_string(result));
	...
	/*********  TEST 2 ****************/
	/* Call Tspi_Context_GetRegisteredKeysByUUID on a leaf key of the SRK */
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_USER,
						      &storage[0].children[0]->uuid,
						      &pulKeyHierarchySize, &ppKeyHierarchy);
	if (result != TSS_SUCCESS) {
		print_error("Tspi_Context_GetRegisteredKeysByUUID ", result);
	...
	/*********  TEST 3 ****************/
	/* Call Tspi_Context_GetRegisteredKeysByUUID on a leaf key of the SRK's child */
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_USER,
						      &storage[1].children[0]->uuid,
						      &pulKeyHierarchySize, &ppKeyHierarchy);
	if (result != TSS_SUCCESS) {
	...
	/*********  TEST 4 ****************/
	/* Call Tspi_Context_GetRegisteredKeysByUUID on a leaf key of the SRK's grandchild */
	result = Tspi_Context_GetRegisteredKeysByUUID(hContext, TSS_PS_TYPE_USER,
						      &storage[2].children[0]->uuid,
						      &pulKeyHierarchySize, &ppKeyHierarchy);
	if (result != TSS_SUCCESS) {
