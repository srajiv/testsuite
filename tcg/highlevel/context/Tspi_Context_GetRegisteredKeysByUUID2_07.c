...
/*
 * NAME
 *	Tspi_Context_GetRegisteredKeysByUUID2_07.c
 *
 * DESCRIPTION
...
void
print_KM_KEYINFO2(TSS_KM_KEYINFO2 *k)
{
	...
	if (k->persistentStorageType == TSS_PS_TYPE_SYSTEM)
		printf("Store        : System\n");
	else if (k->persistentStorageType == TSS_PS_TYPE_USER)
		printf("Store        : User\n");
	else
		printf("Store        : Error\n");

	if (k->persistentStorageTypeParent == TSS_PS_TYPE_SYSTEM)
		printf("Parent Store : System\n");
	else if (k->persistentStorageTypeParent == TSS_PS_TYPE_USER)
		printf("Parent Store : User\n");
	else
		printf("Parent Store : Error\n");
...
main_v1_2(char version){
	...
	char		*nameOfFunction = "Tspi_Context_GetRegisteredKeysByUUID2_07";
	...
	result = Tspi_Context_GetRegisteredKeysByUUID2(hContext, TSS_PS_TYPE_SYSTEM, NULL,
	...
	for (i = 0; i < pulKeyHierarchySize; i++) {
		...
		print_KM_KEYINFO2(&ppKeyHierarchy[i]);
	}
