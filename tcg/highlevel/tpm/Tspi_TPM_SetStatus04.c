 ...
 *
 * DESCRIPTION
 *	This test will verify that Tspi_TPM_SetStatus calls the TPM ResetLockValue
 *	command, which ends the TPM's defense against a dictionary attack.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
...
main_v1_2( void )
{
	...
	result = Tspi_TPM_SetStatus( hTPM, TSS_TPMSTATUS_RESETLOCK, TRUE );
	if ( result != TSS_SUCCESS )
	{
