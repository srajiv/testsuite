...
/*
 * NAME
 *	Tspi_TPM_ReadCurrentTicks01.c
 *
 * DESCRIPTION
 *	This test will verify that Tspi_TPM_ReadCurrentTicks succeeds.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
...
{
	char		  *nameOfFunction = "Tspi_TPM_ReadCurrentTicks-trans03";
	...
	result = Tspi_TPM_ReadCurrentTicks(hTPM, &ticks);
	if (result != TSS_SUCCESS) {
		print_error("Testsuite_Transport_Init", result);
		Tspi_Context_Close(hContext);
		exit(result);
