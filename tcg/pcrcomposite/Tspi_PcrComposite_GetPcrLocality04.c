...
/*
 * NAME
 *	Tspi_PcrComposite_GetPcrLocality04.c
 *
 * DESCRIPTION
 *	This test will verify that Tspi_PcrComposite_GetPcrLocality
 *		returns TSS_E_BAD_PARAMETER when passed one.
 *
 * ALGORITHM
 *	Setup:
 *		Create Context
...
{
	char		*function = "Tspi_PcrComposite_GetPcrLocality04";
	...
	if ( result != TSS_SUCCESS )
	{
		print_error( "Tspi_Context_CreateObject (hPcrComposite)", result );
		print_error_exit( function, err_string(result) );
	...
	result = Tspi_PcrComposite_GetPcrLocality( hPcrComposite, NULL );
